...
from preprocess.TranSalNet_Res import TranSalNet
...
def get_all_saliency(model,data_dir,save_dir):
    folders = [d for d in os.listdir(data_dir) if os.path.isdir(os.path.join(data_dir, d))]
    for i in tqdm(range(len(folders)),desc=f'Extracting saliency samples', total = len(folders)):
        folder_path = os.path.join(data_dir, folders[i])
        frame_size = 20
        for j in range(frame_size):
            img_path = os.path.join(folder_path, '{:03d}'.format(int(j)) + '.png')
            save_path = os.path.join(save_dir, folders[i])
            if not os.path.exists(save_path):
                os.makedirs(save_path)
            save_path = os.path.join(save_dir, folders[i],'{:03d}'.format(int(j)) + '.png')
...
def main(config):
    ...
    get_all_saliency(model,config.data_dir,config.feature_save_folder)
